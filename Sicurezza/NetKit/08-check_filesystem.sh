...
FS_TIPO=`stat -f . | grep -w Tipo`
FS_TIPO=${FS_TIPO#*Tipo: }

if echo $FS_TIPO | grep -vqE "(ext[2,3,4])|(ntfs)|(ntfs-3g)|(fuse)|(reiser)|(jfs)|(xfs)"; then
   echo "failed!"
   echo
   echo "*** Warning: You appear to be running Netkit on a filesystem ($FS_TIPO)"
